...
void network::calculate_output(float in[], float final_out[]){
    ...
    final_out[0] *= 20; // Burn range from 0 - 15: Expert knowledge,
...
void network::print_network_weights() {
    ...
    std::cout << "yeah TESTING";
    for (int i = 0; i < 2; i++) { // step through hidden layers
        for (int j = 0; j < NODES; j++) {
            std::cout << "Layer: " << i << "Node: " << j << std::endl;
            std::cout << "[ ";
            for (int k = 0; k < NODES; k++){
                std::cout << hidden_layer[i].column_of_nodes[j].weights[k]
                        << ", ";
            }
            std::cout << "bias: " << hidden_layer[i].column_of_nodes[j].bias_weight
                    << " ]\n" << std::endl;
    ...
    for (int i = 0; i < 2; i++) {
        for (int k = 0; k < NODES; k++) {
            std::cout << outputs[i].weights[k]
                    << ", ";
        }
        std::cout << "bias: " << outputs[i].bias_weight
                << " ]\n" << std::endl;
    }
}
